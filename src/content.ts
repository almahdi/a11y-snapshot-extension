 ...
 * Collect frame information
 */
function collectFrameInfo(tabId: number): FrameInfoMessage['data'] {
  const frameId = (() => {
 ...
 */
async function sendFrameInfo(tabId: number): Promise<void> {
  try {
    const frameInfo = collectFrameInfo(tabId);
    
    const message: FrameInfoMessage = {
      type: 'FRAME_INFO',
      data: frameInfo,
    };
